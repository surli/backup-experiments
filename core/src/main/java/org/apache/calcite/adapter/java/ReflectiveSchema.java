...
public class ReflectiveSchema
  ...
  @Override protected Map<String, Table> getTableMap() {
    ...
    for (Field field : clazz.getFields()) {
      if (RelOptReferentialConstraint.class.isAssignableFrom(field.getType())) {
        RelOptReferentialConstraint rc;
        try {
          rc = (RelOptReferentialConstraint) field.get(target);
        } catch (IllegalAccessException e) {
          throw new RuntimeException(
              "Error while accessing field " + field, e);
        }
        FieldTable table =
            (FieldTable) tableMap.get(Util.last(rc.getForeignKeyTableQualifiedName()));
        assert table != null;
