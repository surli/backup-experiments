...
public class TestLongDecimalStream
        extends AbstractTestValueStream<BigInteger, DecimalStreamCheckpoint, DecimalOutputStream, DecimalInputStream>
{
    @Test
    public void test()
            throws IOException
    ...
    @Override
    protected void writeValue(DecimalOutputStream outputStream, BigInteger value)
    {
    ...
    {
        return new DecimalInputStream(new OrcInputStream("test", slice.getInput(), SNAPPY, DEFAULT_BUFFER_SIZE, new AggregatedMemoryContext()));
    }

    @Override
    protected BigInteger readValue(DecimalInputStream valueStream)
            throws IOException
    {
        return valueStream.nextBigInteger();
    }
}
