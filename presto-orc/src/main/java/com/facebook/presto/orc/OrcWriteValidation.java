...
public class OrcWriteValidation
{
    ...
    private OrcWriteValidation(List<Integer> version,
    ...
    public void validateFileStatistics(List<ColumnStatistics> actualFileStatistics)
            throws OrcCorruptionException
    {
        validateColumnStatisticsEquivalent("file", actualFileStatistics, fileStatistics);
    }

    public void validateStripeStatistics(List<StripeInformation> actualStripes, List<StripeStatistics> actualStripeStatistics)
            throws OrcCorruptionException
    {
        ...
        if (actualStripeStatistics.size() != stripeStatistics.size()) {
            throw new OrcCorruptionException("Write validation failed: unexpected number of columns in stripe statistics");
        }

        for (int stripIndex = 0; stripIndex < actualStripes.size(); stripIndex++) {
            long stripeOffset = actualStripes.get(stripIndex).getOffset();
            StripeStatistics actual = actualStripeStatistics.get(stripIndex);
            StripeStatistics expected = stripeStatistics.get(stripeOffset);
            if (expected == null) {
                throw new OrcCorruptionException("Unexpected stripe at offset %s", stripeOffset);
            }
            validateColumnStatisticsEquivalent("Stripe at " + stripeOffset, actual.getColumnStatistics(), expected.getColumnStatistics());
        }
    }

    public void validateRowGroupStatistics(long stripeOffset, Map<Integer, List<RowGroupIndex>> actualStripeStatistics)
            throws OrcCorruptionException
    {
        requireNonNull(actualStripeStatistics, "actualStripeStatistics is null");
        ...
        for (Entry<Integer, List<RowGroupIndex>> entry : actualStripeStatistics.entrySet()) {
            if (entry.getValue().size() != rowGroupCount) {
                throw new OrcCorruptionException("Unexpected row group count stripe at offset %s", stripeOffset);
            }
        }
        ...
        for (int rowGroupIndex = 0; rowGroupIndex < expectedRowGroupStatistics.size(); rowGroupIndex++) {
            ...
            if (!expectedStatistics.keySet().equals(actualStripeStatistics.keySet())) {
                throw new OrcCorruptionException("Unexpected column in row group %s in stripe at offset %s", stripeOffset);
            }
            for (Entry<Integer, ColumnStatistics> entry : expectedStatistics.entrySet()) {
                int columnIndex = entry.getKey();
                ColumnStatistics actual = actualStripeStatistics.get(columnIndex).get(rowGroupIndex).getColumnStatistics();
                ColumnStatistics expected = entry.getValue();
                validateColumnStatisticsEquivalent("Stripe at offset " + stripeOffset + " in row group " + rowGroupIndex, actual, expected);
            }
        }
    }

    private static void validateColumnStatisticsEquivalent(String name, List<ColumnStatistics> actualColumnStatistics, List<ColumnStatistics> expectedColumnStatistics)
            throws OrcCorruptionException
    {
        ...
        if (actualColumnStatistics.size() != expectedColumnStatistics.size()) {
            throw new OrcCorruptionException("Write validation failed: unexpected number of columns in %s statistics", name);
        }
        for (int i = 0; i < actualColumnStatistics.size(); i++) {
            ...
            validateColumnStatisticsEquivalent(name + " column " + i, actual, expected);
        }
    }

    private static void validateColumnStatisticsEquivalent(String name, ColumnStatistics actualColumnStatistics, ColumnStatistics expectedColumnStatistics)
            throws OrcCorruptionException
    {
        requireNonNull(name, "name is null");
        requireNonNull(actualColumnStatistics, "actualColumnStatistics is null");
        ...
        if (actualColumnStatistics.getNumberOfValues() != expectedColumnStatistics.getNumberOfValues()) {
            throw new OrcCorruptionException("Write validation failed: unexpected number of values in %s statistics", name);
        }
        if (!Objects.equals(actualColumnStatistics.getBooleanStatistics(), expectedColumnStatistics.getBooleanStatistics())) {
            throw new OrcCorruptionException("Write validation failed: unexpected boolean counts in %s statistics", name);
        }
        if (!Objects.equals(actualColumnStatistics.getIntegerStatistics(), expectedColumnStatistics.getIntegerStatistics())) {
            throw new OrcCorruptionException("Write validation failed: unexpected integer range in %s statistics", name);
        }
        if (!Objects.equals(actualColumnStatistics.getDoubleStatistics(), expectedColumnStatistics.getDoubleStatistics())) {
            throw new OrcCorruptionException("Write validation failed: unexpected double range in %s statistics", name);
        }
        if (!Objects.equals(actualColumnStatistics.getStringStatistics(), trimStringStatisticsForUtf16Ordering(expectedColumnStatistics.getStringStatistics()))) {
            throw new OrcCorruptionException("Write validation failed: unexpected string range in %s statistics", name);
        }
        if (!Objects.equals(actualColumnStatistics.getDateStatistics(), expectedColumnStatistics.getDateStatistics())) {
            throw new OrcCorruptionException("Write validation failed: unexpected date range in %s statistics", name);
        }
        if (!Objects.equals(actualColumnStatistics.getDecimalStatistics(), expectedColumnStatistics.getDecimalStatistics())) {
            throw new OrcCorruptionException("Write validation failed: unexpected decimal range in %s statistics", name);
        }
        if (!Objects.equals(actualColumnStatistics.getBloomFilter(), expectedColumnStatistics.getBloomFilter())) {
            throw new OrcCorruptionException("Write validation failed: unexpected bloom filter in %s statistics", name);
        }
    }

    private static StringStatistics trimStringStatisticsForUtf16Ordering(StringStatistics stringStatistics)
    {
        if (stringStatistics == null) {
            return null;
        }
        return new StringStatistics(
                getMinSlice(stringStatistics.getMin().toStringUtf8()),
                getMaxSlice(stringStatistics.getMax().toStringUtf8()));
    }
