...
public class ContinuousFileProcessingFrom12MigrationTest {
	...
	public void testReaderRestore() throws Exception {
		...
		Assert.assertTrue(testHarness.getOutput().contains(new StreamRecord<>(fsSplit1)));
		Assert.assertTrue(testHarness.getOutput().contains(new StreamRecord<>(fsSplit2)));
		Assert.assertTrue(testHarness.getOutput().contains(new StreamRecord<>(fsSplit3)));
		Assert.assertTrue(testHarness.getOutput().contains(new StreamRecord<>(fsSplit4)));
	}
