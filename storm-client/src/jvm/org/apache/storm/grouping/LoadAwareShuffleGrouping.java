...
public class LoadAwareShuffleGrouping implements LoadAwareCustomStreamGrouping, Serializable {
    ...
    private double HIGHER_BOUND = 0.8;
    private double LOWER_BOUND = 0.2;
    ...
    public void prepare(WorkerTopologyContext context, GlobalStreamId stream, List<Integer> targetTasks) {
        ...
        currentScopeRef = new AtomicReference<>(Scope.WORKER_LOCAL);
        HIGHER_BOUND = ObjectReader.getDouble(conf.get(Config.TOPOLOGY_LOCALITYAWARE_HIGHER_BOUND_PERCENT));
        LOWER_BOUND = ObjectReader.getDouble(conf.get(Config.TOPOLOGY_LOCALITYAWARE_LOWER_BOUND_PERCENT));
    ...
    private Scope transition(LoadMapping load) {
        Scope currentScope = currentScopeRef.get();
    ...
    private synchronized void updateRing(LoadMapping load) {
        ...
        Scope prevScope = currentScopeRef.get();
        Scope curScope = transition(load);
        if (curScope != prevScope) {
            //reset all the weights
            orig.values().stream().forEach(o -> o.resetWeight());
            currentScopeRef.set(curScope);
        }

        List<Integer> targetsInScope = getTargetsInScope(curScope);
