        ...
        implements BlockEncoding
{
    ...
    public MapBlockEncoding(Type keyType, MethodHandle keyNativeHashCode, MethodHandle keyBlockNativeEquals, BlockEncoding keyBlockEncoding, BlockEncoding valueBlockEncoding)
    {
    ...
    public void writeBlock(SliceOutput sliceOutput, Block block)
    {
        ...
        int[] hashTable = mapBlock.getHashTable();

        int valuesStartOffset = offsets[offsetBase];
        int valuesEndOffset = offsets[offsetBase + positionCount];
        keyBlockEncoding.writeBlock(sliceOutput, mapBlock.getKeys().getRegion(valuesStartOffset, valuesEndOffset - valuesStartOffset));
        valueBlockEncoding.writeBlock(sliceOutput, mapBlock.getValues().getRegion(valuesStartOffset, valuesEndOffset - valuesStartOffset));

        sliceOutput.appendInt((valuesEndOffset - valuesStartOffset) * HASH_MULTIPLIER);
        sliceOutput.writeBytes(wrappedIntArray(hashTable, valuesStartOffset * HASH_MULTIPLIER, (valuesEndOffset - valuesStartOffset) * HASH_MULTIPLIER));
    ...
    public Block readBlock(SliceInput sliceInput)
    {
        ...
        return new MapBlock(0, positionCount, valueIsNull, offsets, keyBlock, valueBlock, hashTable, keyType, keyNativeHashCode, keyBlockNativeEquals);
    }
            ...
            implements BlockEncodingFactory<MapBlockEncoding>
    {
        ...
        public MapBlockEncoding readEncoding(TypeManager typeManager, BlockEncodingSerde serde, SliceInput input)
        {
            ...
            return new MapBlockEncoding(keyType, keyNativeHashCode, keyBlockNativeEquals, keyBlockEncoding, valueBlockEncoding);
        }
