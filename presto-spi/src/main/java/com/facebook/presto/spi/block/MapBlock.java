        ...
        extends AbstractMapBlock
{
            ...
            MethodHandle keyNativeHashCode)
    {
        ...
        if (offsets[startOffset + positionCount] - offsets[startOffset] != keyBlock.getPositionCount()) {
            throw new IllegalArgumentException(format("keyBlock/valueBlock size does not match offsets: %s %s %s", keyBlock.getPositionCount(), offsets[startOffset], offsets[startOffset + positionCount]));
        }
