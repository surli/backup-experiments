...
public class DefaultLoggingFactory implements LoggingFactory {
    ...
    private void flushAppender(AsyncAppender appender) throws InterruptedException {
        ...
        if (appender.getNumberOfElementsInQueue() > 0) {
            // It may seem odd to log when we're trying to flush
            // a logger that isn't flushing, but the same warning
            // is issued inside appender.stop() if the appender
            // isn't able to flush.
            appender.addWarn("Possibly " +
                appender.getNumberOfElementsInQueue() + " will be discarded");
        }
    }
