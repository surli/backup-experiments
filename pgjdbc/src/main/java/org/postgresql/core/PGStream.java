  ...
  public PGStream(SocketFactory socketFactory, HostSpec hostSpec, int timeout) throws IOException {
    ...
    if (!socket.isConnected()) {
      try {
        ...
        InetSocketAddress address = (new DefaultProxySelector().select(hostSpec.toURI()).contains(Proxy.NO_PROXY))
        ...
        throw new IOException("Could not create a URI for the socket", e);
      }
    }
    changeSocket(socket);
