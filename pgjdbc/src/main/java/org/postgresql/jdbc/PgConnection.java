...
public class PgConnection implements BaseConnection {
  ...
  public PgConnection(HostSpec[] hostSpecs,
    ...
    if (binaryTransfer && queryExecutor.getProtocolVersion() >= 3) {
      ...
      //the binary representation of TIMESTAMPTZ does not include the known offset value
      //on storage this means presumably offset is always stored as +0
      //on retrieval it means conversion to java.sql.Time or LocalTime is not accurate
    }
