...
public class InputNPathComplexity {
    ...
    void testForCyclesComplex() {
        // NP(for-statement) = (for-range=1) + (expr(1)=0) + (expr(2)=0) + (expr(3)=0) = 1
        for (int i = 0; i < 10; i++);
        // NP(for-statement) = (for-range=1) + (expr(1)=0) + (expr(2)=1) + (expr(3)=0) = 2
        for (int i = 0; i < 10 && true; i++);
        // NP(for-statement) = (for-range=1) + (expr(1)=2) + (expr(2)=0) + (expr(3)=0) = 3
        for (int i = true ? 0 : 0; i < 10; i++);
        // NP(for-statement) = (for-range=1) + (expr(1)=0) + (expr(2)=1+2) + (expr(3)=0) = 4
