...
public class MariaDbPoolDataSourceTest extends BaseTest {
    ...
    public void testIdleTimeout() throws Exception {
        ...
        try (MariaDbPoolDataSource pool = new MariaDbPoolDataSource(connUri + "&maxPoolSize=5&minPoolSize=3&poolName=testIdleTimeout")) {
            ...
            try (Connection connection = pool.getConnection()) {
                //just to ensure pool is initialized
            }
            ...
            List<Long> initialThreadIds = pool.testGetConnectionThreadIds();
            ...
            List<Long> threadIds = pool.testGetConnectionThreadIds();
            assertEquals(initialThreadIds.size(), threadIds.size());
            assertTrue(threadIds.containsAll(initialThreadIds));
            ...
            checkJmxInfo(server, name, 0, 3, 3, 0);
            threadIds = pool.testGetConnectionThreadIds();
            assertEquals(initialThreadIds.size(), threadIds.size());
    ...
    public void ensureUsingPool() throws Exception {
        ...
        connectionAppender.awaitTermination(sharedIsAurora() ? 120 : 30, TimeUnit.SECONDS);
        assertTrue(threadIds.size() <= 8);
        assertTrue(5_000 > System.currentTimeMillis() - start);
