...
public class StreamEngine implements IEngine, IPollEvents
{
    ...
    public StreamEngine(SocketChannel fd, final Options options, final String endpoint)
    {
        ...
        // TODO V4: useless, byte buffers are always allocated in big endian order
        greetingRecv = ByteBuffer.allocate(V3_GREETING_SIZE).order(ByteOrder.BIG_ENDIAN);
        greetingSend = ByteBuffer.allocate(V3_GREETING_SIZE).order(ByteOrder.BIG_ENDIAN);
    ...
    public void plug(IOThread ioThread, SessionBase session)
    {
        ...
        if (options.rawSocket) {
            ...
            encoder = (IEncoder) instantiate(options.encoder, Config.IN_BATCH_SIZE.getValue(), options.maxMsgSize);
            if (encoder == null) {
