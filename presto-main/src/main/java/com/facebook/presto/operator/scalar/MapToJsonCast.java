        ...
        extends SqlOperator
{
    ...
    private static final MethodHandle METHOD_HANDLE = methodHandle(MapToJsonCast.class, "toJson", JsonUtil.MapObjectKeyReader.class, JsonUtil.JsonGeneratorWriter.class, Block.class);
    ...
    public ScalarFunctionImplementation specialize(BoundVariables boundVariables, int arity, TypeManager typeManager, FunctionRegistry functionRegistry)
    {
        ...
        JsonUtil.MapObjectKeyReader reader = JsonUtil.MapObjectKeyReader.createMapObjectKeyReader(keyType);
        JsonUtil.JsonGeneratorWriter writer = JsonUtil.JsonGeneratorWriter.createJsonGeneratorWriter(valueType);
        MethodHandle methodHandle = METHOD_HANDLE.bindTo(reader).bindTo(writer);

        return new ScalarFunctionImplementation(false, ImmutableList.of(false), methodHandle, isDeterministic());
    }

    @UsedByGeneratedCode
    public static Slice toJson(JsonUtil.MapObjectKeyReader reader, JsonUtil.JsonGeneratorWriter writer, Block block)
    {
        try {
            Map<String, Integer> orderedKeyToValuePosition = new TreeMap<>();
            for (int i = 0; i < block.getPositionCount(); i += 2) {
                String objectKey = reader.read(block, i);
            ...
            try (JsonGenerator jsonGenerator = createJsonGenerator(JSON_FACTORY, output)) {
                ...
                for (Map.Entry<String, Integer> entry : orderedKeyToValuePosition.entrySet()) {
                    ...
                    writer.writeObjectKeyValuePair(jsonGenerator, entry.getKey(), block, valuePosition);
                }
