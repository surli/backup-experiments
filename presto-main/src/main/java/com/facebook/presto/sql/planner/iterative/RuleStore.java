...
public class RuleStore
{
    ...
    {
        return Streams.stream(superPlanNodes(planNode.getClass()))
                .flatMap(clazz -> rulesByClass.get(clazz).stream());
    }

    private static Iterator<Class<? extends PlanNode>> superPlanNodes(Class<? extends PlanNode> planNodeClass)
    {
        return new Iterator<Class<? extends PlanNode>>()
        {
            Class clazz = planNodeClass;

            @Override
            public boolean hasNext()
            {
                return PlanNode.class.isAssignableFrom(clazz);
            }

            @SuppressWarnings("unchecked")
            @Override
            public Class<? extends PlanNode> next()
            {
                Class<? extends PlanNode> currentClazz = clazz;
