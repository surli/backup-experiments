...
public class QueryExplainer
{
    ...
    {
        Analyzer analyzer = new Analyzer(session, metadata, sqlParser, accessControl, Optional.of(this), parameters, BlackholeWarningSink.getInstance());
    ...
    public String getGraphvizPlan(Session session, Statement statement, Type planType, List<Expression> parameters)
    {
        ...
        WarningSink warningSink = BlackholeWarningSink.getInstance();
        switch (planType) {
            case LOGICAL:
                Plan plan = getLogicalPlan(session, statement, parameters, warningSink);
    ...
    private Plan getLogicalPlan(Session session, Statement statement, List<Expression> parameters, WarningSink warningSink)
    {
        // analyze statement
        ...
        // plan statement
        LogicalPlanner logicalPlanner = new LogicalPlanner(session, planOptimizers, idAllocator, metadata, sqlParser, warningSink);
