...
public class PageFunctionCompiler
{
    ...
    private ClassLoader pageProjectionOutputClassLoader;
    ...
    private Supplier<PageProjection> compileProjectionInternal(RowExpression projection, Optional<String> classNameSuffix)
    {
        ...
        try {
            projectionClass = defineClass(classDefinition, PageProjection.class, callSiteBinder.getBindings(), pageProjectionOutputClassLoader);
        }
        catch (Exception e) {
            ...
            Optional<String> classNameSuffix)
    {
        ...
        // project
        generatePageProjectMethod(classDefinition, blockBuilderField, projection);
    ...
    private MethodDefinition generatePageProjectMethod(ClassDefinition classDefinition, FieldDefinition blockBuilder, RowExpression projection)
    {
        ...
        CallSiteBinder callSiteBinder = new CallSiteBinder();
        ClassDefinition pageProjectionOutputDefinition = definePageProjectOutputClass(projection, callSiteBinder);
        Class<? extends PageProjectionOutput> pageProjectionOutputClass = defineClass(pageProjectionOutputDefinition, PageProjectionOutput.class, callSiteBinder.getBindings(), getClass().getClassLoader());
        pageProjectionOutputClassLoader = pageProjectionOutputClass.getClassLoader();

        Variable pageProjectionOutput = scope.declareVariable(PageProjectionOutput.class, "pageProjectionOutput");
        body.append(pageProjectionOutput.set(newInstance(pageProjectionOutputClass, thisVariable.getField(blockBuilder), session, yieldSignal, page, selectedPositions)))
                .append(pageProjectionOutput.ret());

        return method;
    }

    private ClassDefinition definePageProjectOutputClass(RowExpression projection, CallSiteBinder callSiteBinder)
    {
        ClassDefinition classDefinition = new ClassDefinition(
                a(PUBLIC, FINAL),
                makeClassName(PageProjectionOutput.class.getSimpleName()),
                type(Object.class),
                type(PageProjectionOutput.class));
