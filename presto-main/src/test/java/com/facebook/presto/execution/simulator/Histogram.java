...
class Histogram<K extends Comparable<K>>
{
    ...
    static <K extends Comparable<K>> Histogram<K> fromDiscrete(Collection<K> buckets)
    {
        return new Histogram<>(buckets, true);
    }

    static <K extends Comparable<K>> Histogram<K> fromContinuous(Collection<K> buckets)
    {
        return new Histogram<>(buckets, false);
    }

    static <D> Histogram<Long> fromContinuous(Collection<D> initialData, Function<D, Long> keyFunction)
    {
    ...
    <D, V, F, G> void printDistribution(
            Collection<D> data,
            Function<D, K> keyFunction,
            ...
            Function<List<V>, G> valueFormatter)
    {
        ...
        if (!discrete) {
            ...
            for (int i = 0; i < bucketData.size(); i++) {
                System.out.printf("%8s : (%5s values) %s\n",
                        keyFormatter.apply(buckets.get(i)),
                        bucketData.get(i).size(),
                        valueFormatter.apply(bucketData.get(i)));
            }
