...
import com.facebook.presto.execution.simulator.SplitSpecification.IntermediateSplitSpecification;
import com.facebook.presto.execution.simulator.SplitSpecification.LeafSplitSpecification;
...
{
    private static final int SPLITS_PER_LEAF_TASK = 4;
    private static final int SPLITS_PER_INTERMEDIATE_TASK = 8;
    ...
    public static void main(String[] args)
    {
        ...
        System.out.println("============================");
        System.out.println();
        System.out.println("Tasks with " + SPLITS_PER_INTERMEDIATE_TASK + "x " + IntermediateSplitGenerator.class.getSimpleName());
        ...
        for (SplitGenerator generator : leafSplitGenerators) {
            ...
            System.out.println();
            System.out.println("Tasks with " + SPLITS_PER_LEAF_TASK + "x " + generator.getClass().getSimpleName());
    ...
    static class IntermediateSplitGenerator
    ...
    static class AggregatedLeafSplitGenerator
            implements SplitGenerator
    {
    ...
    static class FastLeafSplitGenerator
            implements SplitGenerator
    {
        public LeafSplitSpecification next()
        {
    ...
    static class SlowLeafSplitGenerator
            implements SplitGenerator
    {
        public LeafSplitSpecification next()
        {
    ...
    static class L4LeafSplitGenerator
            implements SplitGenerator
    {
        public LeafSplitSpecification next()
        {
    ...
    static class QuantaExceedingSplitGenerator
            implements SplitGenerator
    {
        public LeafSplitSpecification next()
        {
    ...
    static long generateLeafSplitScheduledTimeMs(double origin, double bound)
    {
    ...
    static long generateLeafSplitPerCallMicros(double origin, double bound)
    {
        ThreadLocalRandom generator = ThreadLocalRandom.current();
        double value = generator.nextDouble(origin, bound);
        if (value > 0.9999) {
    ...
    static long generateIntermediateSplitScheduledTimeMs(double origin, double bound)
    {
        ThreadLocalRandom generator = ThreadLocalRandom.current();
        double value = generator.nextDouble(origin, bound);
        // in reality, max is several hours, but this would make the simulation too slow
    ...
    static long generateIntermediateSplitWallTimeMs(double origin, double bound)
    {
        ThreadLocalRandom generator = ThreadLocalRandom.current();
        double value = generator.nextDouble(origin, bound);
        // in reality, max is several hours, but this would make the simulation too slow
    ...
    static long generateIntermediateSplitNumQuanta(double origin, double bound)
    {
        ThreadLocalRandom generator = ThreadLocalRandom.current();
        double value = generator.nextDouble(origin, bound);
