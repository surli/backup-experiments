...
public class ByteBufferHashTable
{
  ...
  // current number of available buckets in the table
  ...
  public void adjustTableWhenFull()
  {
    ...
    for (int oldBucket = 0; oldBucket < oldBuckets; oldBucket++) {
      if (isBucketUsed(oldBucket)) {
        ...
        final int newBucket = findBucket(canAllowNewBucket(), newTableBuffer, keyBuffer, keyHash);

        if (newBucket < 0) {
          throw new ISE("WTF?! Couldn't find a bucket while resizing?!");
        }

        newTableBuffer.position(newBucket * bucketSizeWithHash);
        newTableBuffer.put(entryBuffer);

        buffer.putInt(tableArenaSize + newSize * Ints.BYTES, newBucket * bucketSizeWithHash);
        newSize++;
      }
    }
  ...
  {
    int bucket = findBucket(canAllowNewBucket(), tableBuffer, keyBuffer, keyHash);

    if (bucket < 0) {
      if (size < maxSizeForTesting) {
        adjustTableWhenFull();
        bucket = findBucket(size < maxSize, tableBuffer, keyBuffer, keyHash);
      }
    }
...
outer:
  ...
  private boolean canAllowNewBucket()
  {
